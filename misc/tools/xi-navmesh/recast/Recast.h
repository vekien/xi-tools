...
public:
	...
	inline void start_timer(const rcTimerLabel label) { if (m_timerEnabled) doStartTimer(label); }
...
public:
	...
	///  @param[in]		label	The category of the timer.
	inline rcScopedTimer(rcContext* ctx, const rcTimerLabel label) : m_ctx(ctx), m_label(label) { m_ctx->start_timer(m_label); }
