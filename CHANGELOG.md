...
- **A job ability or spell can carry a unique baked motion.** Publishing a recipe whose motion is race-bound (an emote, a weapon skill, a race's own file) as `ja` or `spell` no longer errors or forces `ws`: compose bakes the motion from one race's copy (the `--race`, HumeMale by default) into the single DAT and carries its clips — the way retail Blue Magic ships its own `wz*` clips, playing on every race. `xi ability compose` gains `--kind`; the mixer offers every motion on a job ability or spell and notes when one will be baked. Weapon-skill *animation* slots are unchanged (16 per race; cexislots raises the spell / job-ability record ceilings, not that motion bank)
