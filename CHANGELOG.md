...
- **A custom-band weapon skill registers its waist companions.** Published from motion that is not a weapon skill (an emote, a battle pack), a number in the custom band left each race's two waist ids (body + 8 and + 16) unregistered, since there was no source slot to copy them from — and retail never leaves a slot without them. They now get retail's placeholder, slot 0's companions: a 160-byte `[dumm]` DAT, byte for byte the same on every race. The number then registers 24 DATs (three per race), and `dats build --dry-run` lists them. A retail slot (264–271) keeps its own placeholders, as before
