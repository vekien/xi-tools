...
- **Custom animation bands** (Carver): with `FX_*_BAND_*` set in `.env` to match a client-side plugin (cexislots), the publisher allocates past the retail ceilings — spells, job abilities, and a third weapon-skill motion bank (272–527 with cexislots, 24 file ids per number) once 264–271 are taken. The "no free slot" error now says so
