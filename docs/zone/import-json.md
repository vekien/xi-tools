...
| `add` *(animated / generator-bound)* | With `anim` (`{"source_id": "f001", "source_dat": "ROM2/12/123.DAT", "source_offset": 5700064, "new_id"?: "f003"}`), the copy is a **generator-bound object** (see [format.md → Generator-bound objects](format.md#generator-bound-objects-animated-placements)): the named `0x05` generator is cloned under a fresh FourCC (same-DAT duplicate, or from `source_dat` with its non-mesh deps), re-targeted at the copy's `pos`/`rot`/`scale` and at the copy's `0x2E` mesh (the `xi_`-prefixed import on a cross-zone copy), and that FourCC is written into the new record's BlockID `+0x34`. The bridge pins the allocated id back as `anim.new_id` so later publishes reuse it. Extra `rotation` / `rot_velocity` / `rotation_order` keys are the editor's preview data and are ignored here. |
| `modify` / `delete` *(of a bound record)* | A record whose BlockID names a generator is drawn by that generator, so `modify` also rewrites the generator's base position, `0x09` rotation and `0x0F` scale, and `delete` also clears the generator's `autoRun` flag and parks its base position out of the world. |
...
| Op | Behaviour |
|---|---|
| `modify` | Patch position and/or scale (and `rot` → sec2 `0x09` Rotation) in the `0x05` section body |
